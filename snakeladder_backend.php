...
function check_inactive_and_end($room, $roomId) {
    if (is_room_inactive($room, room_path($roomId), game_inactivity_timeout_seconds('snakeladder'))) {
...
function write_status_file($room, $event) {
    ...
    update_main_status('snakeladder', $state);
}
...
switch ($action) {
    ...
    case 'back': {
        ...
        if ($action === 'leave') {
            respond(player_leave_room('snakeladder', $roomId, $playerId, !empty($_REQUEST['soft'])));
        }
